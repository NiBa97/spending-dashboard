...
const UpdatePaymentSchedule = ({
  ...
  const { mutate, isLoading: isPosting } =
    api.paymentSchedule.update.useMutation({
      onSuccess: () => {
  ...
  return (
    <div className="flex w-full flex-col gap-3 rounded bg-white p-4 shadow">
      <h1 className="text-center text-2xl font-bold">Update Paymentschedule</h1>
      <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          <input
            ...
            name="startDate"
            onChange={handleChange}
            value={paymentSchedule.startDate.toISOString().slice(0, 10)}
          />
        </div>
        <div>
          <label className="block text-sm font-bold text-gray-700">
      ...
      {!isPosting && (
        <button
          ...
          onClick={() =>
            mutate({
              ...paymentSchedule,
              lastRun: paymentSchedule.lastRun || undefined,
            })
          }
        >
          Post
...
const CreatePaymentSchedule = ({
  ...
  onPaymentScheduleCreation: () => void;
}) => {
  // Give me the next auth user session
  const { data: sessionData } = useSession();
  ...
  const { mutate, isLoading: isPosting } =
    api.paymentSchedule.create.useMutation({
      onSuccess: () => {
        void ctx.paymentSchedule.getAll.invalidate();
        onPaymentScheduleCreation();
      },
  ...
  return (
    <div className="flex w-full flex-col gap-3 rounded bg-white p-4 shadow">
      <h1 className="text-center text-2xl font-bold">Create Payment</h1>
      <div className="flex flex-col gap-3">
        <div>
          <label className="block text-sm font-bold text-gray-700">Name:</label>
          <input
      ...
      {!isPosting && (
        <button
          className="mt-4 rounded bg-blue-500 px-4 py-2 font-bold text-white hover:bg-blue-700"
          onClick={() => mutate({ ...paymentSchedule })}
        >
          Post
        </button>
      )}
...
const CreatePaymentScheduleModal = (props: {
  ...
  return (
    <div>
      {props.paymentSchedule ? (
        ...
        <button className="text-white" onClick={() => setModalIsOpen(true)}>
      ...
      >
        {props.paymentSchedule ? (
          <UpdatePaymentSchedule
            _paymentSchedule={props.paymentSchedule}
            onPaymentUpdate={handlePaymentScheduleCreation}
          />
        ) : (
          <CreatePaymentSchedule
            onPaymentScheduleCreation={handlePaymentScheduleCreation}
          />
        )}
      </Modal>
    </div>
...
const ViewPaymentSchedule = (props: { paymentSchedule: PaymentSchedule }) => {
  const amountColor =
    props.paymentSchedule.amount < 0 ? "text-red-500" : "text-green-500";
  const ArrowIcon = props.paymentSchedule.amount < 0 ? FaArrowDown : FaArrowUp;

  const ctx = api.useContext();

  const { mutate, isLoading: isPosting } =
    api.paymentSchedule.delete.useMutation({
      onSuccess: () => {
        void ctx.paymentSchedule.getAll.invalidate();
  ...
  return (
    <div className="transform rounded-lg bg-white p-4 shadow-lg transition-transform duration-500 hover:scale-105">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <FaCalendarAlt className="text-md" />
          <p className="text-md">
            {props.paymentSchedule.startDate.toDateString()}
          </p>
        </div>
      ...
      <div className="flex items-end justify-center">
        <div className="flex items-center space-x-2">
          ...
          <p className={`text-center text-2xl ${amountColor} align-top`}>€</p>
        </div>
      </div>
      <div className="mt-4">
  ...
  return (
    <div>
      ...
      >
        {props.payment ? (
          <UpdatePayment
            _payment={props.payment}
            onPaymentUpdate={handlePaymentCreation}
          />
        ) : (
          <CreatePayment onPaymentCreation={handlePaymentCreation} />
        )}
      </Modal>
    </div>
  );
...
const UpdatePayment = ({ _payment, onPaymentUpdate }: UpdatePaymentProps) => {
  ...
  return (
    <div className="flex w-full flex-col gap-3 rounded bg-white p-4 shadow">
      <h1 className="text-center text-2xl font-bold">Update Payment</h1>
      <div className="flex flex-col gap-3">
        <div>
          <label className="block text-sm font-bold text-gray-700">Name:</label>
          <input
...
export default function Home() {
  ...
  const { mutate: mutatePaymentSchedule } =
    api.paymentSchedule.update.useMutation();
  useEffect(() => {
    if (PaymentSchedules) {
  ...
  return (
    <>
      <div className="container flex flex-col items-center justify-center gap-12 px-4 py-16 ">
        ...
        {PaymentSchedules && (
          <div className="grid grid-cols-5 gap-4 duration-1000 md:grid-cols-2 lg:grid-cols-5">
        ...
        <Accordion className="w-full">
          {Object.entries(groupedPayments).map(([month, payments]) => (
            <AccordionItem key={month}>
              <AccordionItemHeading className="my-2 rounded bg-white p-3">
                <AccordionItemButton>
                  <span>{month}</span>
                  <span className="float-right ml-3 text-sm text-gray-500">
                    {calculateSum(payments as Payment[])} €
                  </span>
                </AccordionItemButton>
              </AccordionItemHeading>
              <AccordionItemPanel className="transition-all duration-1000">
                <div className="grid grid-cols-5 gap-4 duration-1000 md:grid-cols-2 lg:grid-cols-5">
